...
const checkStorage = () => {
  if (typeof Storage == undefined) {
    alert("Web browser kamu tidak suport");
    return false;
  }
...
const refreshDataFromBooks = () => {
  ...
  for (book of books) {
    const TambahBook = BuatBook(book.bookTitle, book.bookAuthor, book.bookYear, book.isCompleted);
    TambahBook[BOOK_ITEMID] = book.id;

    if (book.isCompleted) {
      bookCompleted.append(TambahBook);
    } else {
      bookUncompleted.append(TambahBook);
    }
  }
};
